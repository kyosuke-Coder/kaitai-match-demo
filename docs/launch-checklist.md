...
## ★最速マネタイズ・ルート（72時間で「売っている状態」へ）

お金をかけず・審査待ちなしで現金化の口を開ける順番。ドメインもサイトも後回しでいい。

**今夜（30分）**
- [ ] Gumroadアカウント作成（メールだけ・審査なし）→ `Tokyo-Travel-Planner-v0.2.pdf` を$5で出品（出品文・画像はこのリポジトリからコピペ: `etsy-listing-draft.md` のGumroad版セクション）
- [ ] これで「売れる状態」が完成。リンクが手に入る

**明日の夜（30分）**
- [ ] Pinterestビジネスアカウント開設 → pin1/pin2を投稿、リンク先はGumroad
- [ ] 就業規則の副業規定を確認（並行で）

**週末（半日）**
- [ ] Etsyショップ開設（本人確認・銀行/Payoneer接続）→ 同商品を$4.90で出品 ＋ Etsy内広告$1/日
- [ ] Cloudflare Pagesに `site/` をデプロイ（ドメインは後からでも可・まず *.pages.dev で公開）→ サイトのボタンをGumroad/Etsy実URLに差し替え

**2週間後**
- [ ] 初計測（表示・お気に入り・購入）→ 数字で次の一手
